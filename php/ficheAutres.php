<?php
    ...
    $req = $bd->prepare("SELECT  designSauna,date_format(dateSauna,'%d-%m-%Y') as dateSauna,accompte FROM sauna WHERE  dateSauna LIKE '%$dateJ%'");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM sauna WHERE dateSauna LIKE '%$dateJ%'  ");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT  designSauna,date_format(dateSauna,'%d-%m-%Y') as dateSauna,accompte FROM sauna WHERE  dateSauna BETWEEN ? AND ? ");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM sauna WHERE dateSauna BETWEEN ? AND ?  ");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h1 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche d'entrées Autres services
