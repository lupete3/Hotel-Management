	      		...
	      		<div class="col-md-4">
	      			<h3>
	      				Total des dettes: <strong><span class="badge badge-danger btn-lg">
	      					<?php 
	      					$req = $bd->query("SELECT SUM(reste) as total FROM location AS A ,organisation AS B, salle as C WHERE  B.idOrg=A.idOrg AND C.idSalle=A.idSalle AND A.modepaie='Credit' AND A.type='Crédit' ");
	 				...
	 				<table id="tab" class="display table table-bordered table-striped table-sm">
		          	...
		          	<tbody>
		          		<?php 
		                    ...
		                    $req = $bd->query("SELECT * FROM location AS A ,organisation AS B, salle as C WHERE  B.idOrg=A.idOrg AND C.idSalle=A.idSalle AND A.modepaie='Credit' AND A.type='Crédit' GROUP BY B.idOrg ");
		                
		          			while ($don=$req->fetch(PDO::FETCH_ASSOC)) {
		          				?>
								<tr>
	                               <td><?php echo $don['idOrg']; ?></td>
	                               <td><?php echo $don['nomOrg'] ;?></td>
	                               <td><?php echo $don['adresse']; ?></td>
	                                <td><?php echo $don['emailOrg']; ?></td>
	                               <td><?php echo $don['tel']; ?></td>
	                               <td><?php echo $don['nomSalle']; ?></td>
	                               <td>
										<a href="../paiementDetteOrg.php?idOrg=<?php echo $don['idOrg'];?>" data-toggle="tooltip" title="Cliquer pour visualiser cette créance">
