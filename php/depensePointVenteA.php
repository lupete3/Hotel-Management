<?php
    ...
    $req = $bd->prepare("SELECT  agentDp,montantDp,date_format(dateDp,'%d-%m-%Y') as dateDp,motifDp FROM depensepdv WHERE  dateDp LIKE '%$dateJ%'");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(montantDp) AS total FROM depensepdv WHERE dateDp LIKE '%$dateJ%'");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT  agentDp,montantDp,date_format(dateDp,'%d-%m-%Y') as dateDp,motifDp FROM depensepdv WHERE  dateDp BETWEEN ? AND ? ");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(montantDp) AS total FROM depensepdv WHERE dateDp BETWEEN ? AND ? ");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h4 class="text-center text-uppercase" style="text-align:center">Fiche de depenses points de vente
