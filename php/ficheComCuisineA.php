<?php
    ...
    $dateJ=date('Y-m-d');
    $statut='Validé';
    $req = $bd->query("SELECT * FROM commandenour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND dateComNour LIKE '%$dateJ%' AND statutComNour='$statut'");

    $req1=$bd->query("SELECT SUM(qteComNour*puNour) as total FROM commandenour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND dateComNour LIKE '%$dateJ%' AND statutComNour='$statut'");
    if (isset($_GET['save'])) {
        $in=$_GET['dateIn'];
        $out=$_GET['dateOut'];

         $req = $bd->prepare("SELECT * FROM commandenour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND dateComNour BETWEEN ? AND ? AND statutComNour='$statut'");
         $req->execute(array($in,$out));
        $req1=$bd->prepare("SELECT SUM(qteComNour*puNour) as total FROM commandenour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND dateComNour BETWEEN ? AND ? AND statutComNour='$statut'");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h3 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Liste de commandes journalière de  la cuisine </h3>
                        <div class="cacher">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                    ...
                                    <td><?php echo $don['qteComNour']* $don['puNour'] ?></td>
                                    <td></td>
