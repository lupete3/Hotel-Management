<?php
    ...
    if (isset($_GET['save'])) {
        ...
        $out=$_GET['dateOut'];
         ...
         $req = $bd->prepare("SELECT * FROM avarienour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND A.dateNourAv BETWEEN ? AND ? AND postePvNour = 'cuisinier' ");
         $req->execute(array($in,$out));
        $req1=$bd->prepare("SELECT SUM(qteNourAv * puNour) as total FROM avarienour AS A ,catnour AS B , prodnour AS C WHERE A.idNour=C.idNour AND C.idCatNour=B.idCatNour AND A.dateNourAv BETWEEN ? AND ? AND postePvNour = 'cuisinier' ");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h3 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche des avaries journalières de  la cuisine </h3>
                        <div class="cacher">
