<?php
    ...
    require_once('bd_cnx.php');
        ...
        $req = $bd->prepare("SELECT  designSauna,date_format(dateSauna,'%d-%m-%Y') as dateSauna,accompte FROM sauna WHERE  dateSauna LIKE '%$in%'");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM sauna WHERE dateSauna LIKE '%$in%'");
        $req1->execute();
