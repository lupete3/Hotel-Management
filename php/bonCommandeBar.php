<?php
    ...
    require_once('bd_cnx.php');
        ...
        $req=$bd->query("SELECT A.idCom,A.qteCom,date_format(A.dateCom,'%e-%m-%Y à %T') as dateCom,designBoiss,qteCom,nbUniteBoiss,puBoiss,A.idBoiss FROM commande as A,pointvente as B,prodboiss as C WHERE A.idBoiss=C.idBoiss AND A.idPv=B.idPv AND A.statutCom = 'commande' AND B.idPv='$c'");
        $req1=$bd->query("SELECT SUM(qteCom*nbUniteBoiss*puBoiss) as total FROM commande as A,pointvente as B,prodboiss as C WHERE A.idBoiss=C.idBoiss AND A.idPv=B.idPv AND A.statutCom = 'commande' AND B.idPv='$c'");
...
<html>
    ...
    <body id="">
        <div class="container" style="border:1px solid gray; border-radius:20px; margin-bottom:10px; ">
            ...
            <div class="row" style="margin-bottom:10px;  " >
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    ...
                                    <tbody>
                                        <?php 
                                            ...
                                            while ($don=$req->fetch(PDO::FETCH_ASSOC)) {
                                                $id = $don['idBoiss'];
                                                 $req2=$bd->query("SELECT qtStock FROM stockpv where idBoiss = '$id'");
                                                 $don1 = $req2->fetch();
                                                ?>
                                                <tr>
                                                    <td><?php echo $num;  ?></td>
                                                    <td><?php echo $don['designBoiss'] ?></td>
                                                    <td class="text-right"><?php echo $don['qteCom'] ?></td>
                                                    <td class="text-right"><?php echo $don['nbUniteBoiss'] ?></td>
                                                    <td class="text-right"><?php echo $don['puBoiss'] ?></td>
                                                    <td class="text-right"><?php echo $don['qteCom']*$don['nbUniteBoiss']*$don['puBoiss'] ?></td>
                                                    <td class="text-right"><?php echo $don1['qtStock']?></td>
                                                </tr>
                                                <?php
                                                $num++;
                                            }
                    ...
                    <div class="container">
                        <div class="row">
                                ...
                                <?php if (isset($_POST['send'])): 
                                    ?>
                                <?php
                                    $req=$bd->query("UPDATE commande SET statutCom='Encours' WHERE statutCom='commande'"); 
                                    header('location:pages/comBoiss.php');
                            endif ?>
                                <div class="col-lg-6">
                    ...
                    <div class="col-lg-12" style="">
                             <p style="font-size:1em; font-family:Century Gothic;">Signature barman</p> 
                    </div>
            </div>
                    </div>
