...
<?php
    ...
    $req1=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier WHERE datePanier LIKE '%$dateJ%' AND product='B' AND etatPanier='close'");
    $req1->execute(array());
    $don1=$req1->fetch();
    
    $req3=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier WHERE datePanier LIKE '%$dateJ%' AND product='B'  AND etatPanier='close' AND modePaie='Cash'");
    $req3->execute(array());
    $don3=$req3->fetch();

    $req4=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier WHERE datePanier LIKE '%$dateJ%' AND product='B'  AND etatPanier='close' AND modePaie='Crédit'");
    $req4->execute(array());
    $don4=$req4->fetch();

    $req2=$bd->query("SELECT designation,qtePanier,puPanier,ptPanier,modePaie,date_format(datePanier, '%d-%m-%Y à %T ') as datePanier FROM panier  WHERE datePanier LIKE '%$dateJ%' AND product='B' AND etatPanier='close' ORDER BY  datePanier DESC ");
    if (isset($_GET['save'])) {
        $in=$_GET['dateIn'];
        $out=$_GET['dateOut'];
        $pointvente=$_GET['pdv'];

         $req1 = $bd->prepare("SELECT SUM(ptPanier) as total FROM panier  WHERE datePanier BETWEEN ? AND ? AND product='B' AND etatPanier='close' AND idPv='$pointvente'");
         $req1->execute(array($in,$out));
         $don1=$req1->fetch();

         $req3=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier WHERE datePanier BETWEEN ? AND ? AND product='B'  AND etatPanier='close' AND modePaie='Cash' AND idPv='$pointvente'");
        $req3->execute(array($in,$out));
        $don3=$req3->fetch();

        $req4=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier WHERE datePanier BETWEEN ? AND ? AND product='B'  AND etatPanier='close' AND modePaie='Crédit' AND idPv='$pointvente'");
        $req4->execute(array($in,$out));
        $don4=$req4->fetch();


        $req2=$bd->prepare("SELECT designation,qtePanier,puPanier,ptPanier,modePaie,date_format(datePanier, '%d-%m-%Y à %T ') as datePanier FROM panier  WHERE datePanier BETWEEN ? AND ? AND product='B' AND etatPanier='close' AND idPv='$pointvente' ORDER BY  datePanier DESC");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h4 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche de Vente Boissons <?php if(isset($_GET['save'])){
                            $pdv=$_GET['pdv'];
