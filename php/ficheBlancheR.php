<?php
    ...
    $dateJ=date('Y-m-d');
     ...
     $diff='BL';
        $req = $bd->prepare("SELECT  designActivite,date_format(dateFact,'%d-%m-%Y') as dateFact,accompte FROM facturation WHERE  dateFact LIKE '%$dateJ%' AND diffIndex=?");
        $req->execute(array($diff));
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM facturation WHERE dateFact LIKE '%$dateJ%'  AND diffIndex=? ");
        $req1->execute(array($diff));
    ...
    if (isset($_GET['save'])) {
        ...
        $diff='BL';
        $req = $bd->prepare("SELECT  designActivite,date_format(dateFact,'%d-%m-%Y') as dateFact,accompte FROM facturation WHERE  dateFact BETWEEN ? AND ? AND diffIndex=?");
        $req->execute(array($in,$out,$diff));
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM facturation WHERE dateFact BETWEEN ? AND ?  AND diffIndex=? ");
        $req1->execute(array($in,$out,$diff));
        $don1=$req1->fetch(PDO::FETCH_ASSOC);
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h1 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche d'entrées blanchisserie
