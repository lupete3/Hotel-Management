...
<?php
    ...
    $dateJ = date('Y-m-d');
    $req1=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier AS A INNER JOIN pointvente AS B ON B.idPv=A.idPv WHERE A.datePanier LIKE '%$dateJ%' AND product='B' AND etatPanier='close' AND B.idPv='$point'");
    $req1->execute(array());
    $don1=$req1->fetch();
    

    $req2=$bd->query("SELECT * FROM panier AS A INNER JOIN pointvente AS B ON B.idPv=A.idPv  WHERE A.datePanier LIKE '%$dateJ%' AND product='B' AND etatPanier='close' AND B.idPv='$point' ORDER BY  datePanier DESC ");

    $req3=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier AS A INNER JOIN pointvente AS B ON B.idPv=A.idPv WHERE A.datePanier LIKE '%$dateJ%' AND product='B'  AND etatPanier='close' AND modePaie='Cash' AND B.idPv='$point'");
    $req3->execute(array());
    $don3=$req3->fetch();

    $req4=$bd->prepare("SELECT SUM(ptPanier) as total FROM panier AS A INNER JOIN pointvente AS B ON B.idPv=A.idPv WHERE A.datePanier LIKE '%$dateJ%' AND product='B'  AND etatPanier='close' AND modePaie='Crédit' AND B.idPv='$point'");
    $req4->execute(array());
    $don4=$req4->fetch();
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h3 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche de Vente Boissons/<?php echo $poste; ?>
