<?php
    ...
    $dateJ=date('Y-m-d');
    $req = $bd->prepare("SELECT B.nomFour,date_format(A.dateApprov,'%d-%m-%Y') as dateApprov,A.qnteApprov,A.puApprov,C.designNour FROM approvnour as A,four as B,prodnour as C WHERE C.idNour=A.idNour AND B.idFour=A.idFour AND A.dateApprov Like '%$dateJ%'");
        $req->execute();
