<?php
    ...
    require_once('bd_cnx.php');
        ...
        $req = $bd->prepare("SELECT designActivite,date_format(dateFact,'%d-%m-%Y') as dateFact,accompte FROM facturation WHERE  dateFact LIKE '%$in%' AND diffIndex=?");
        $req->execute(array($diff));
        $req1 = $bd->prepare("SELECT SUM(accompte) AS total FROM facturation WHERE  dateFact LIKE '%$in%' AND diffIndex=?");
