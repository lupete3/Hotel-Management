...
<?php
    ...
    $req = $bd->prepare("SELECT C.qnteSort,date_format(C.dateSort,'%e-%m-%Y à %T') as dateSort,A.libPv, B.designBoiss,B.puBoiss, B.idBoiss,B.nbUniteBoiss FROM sortieboiss as C,prodboiss as B,pointvente as A WHERE A.idPv=C.idPv AND  C.dateSort LIKE '%$dateJ%' AND C.idBoiss = B.idBoiss");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(qnteSort*puBoiss*nbUniteBoiss) AS total FROM sortieboiss as C,prodboiss as B,pointvente as A WHERE A.idPv=C.idPv AND  C.dateSort LIKE '%$dateJ%' AND C.idBoiss = B.idBoiss");
        $req1->execute();
        $don1=$req1->fetch(PDO::FETCH_ASSOC);
        $req2=$bd->prepare('SELECT designBoiss FROM prodboiss');
        $req2->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT C.qnteSort,date_format(C.dateSort,'%e-%m-%Y à %T') as dateSort,A.libPv, B.designBoiss,B.puBoiss,B.nbUniteBoiss, B.idBoiss FROM sortieboiss as C,prodboiss as B,pointvente as A WHERE A.idPv=C.idPv AND  C.dateSort BETWEEN ? AND ? AND C.idBoiss = B.idBoiss AND A.idPv=?");
        $req->execute(array($in,$out,$pointvente));
        $req1 = $bd->prepare("SELECT SUM(qnteSort*puBoiss*nbUniteBoiss) AS total FROM sortieboiss as C,prodboiss as B,pointvente as A WHERE A.idPv=C.idPv AND  C.dateSort BETWEEN ? AND ? AND C.idBoiss = B.idBoiss AND A.idPv=?");
        $req1->execute(array($in,$out,$pointvente));
        $don1=$req1->fetch(PDO::FETCH_ASSOC);
        $req2=$bd->prepare('SELECT designBoiss FROM prodboiss');
        $req2->execute();
        $don2=$req2->fetch(PDO::FETCH_ASSOC);
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h3 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche Sortie boissons <?php if(isset($_GET['save'])){
                            $pdv=$_GET['pdv'];
                    ...
                    <table class="table table-bordered table-striped spacer">
                        ...
                        <tbody>
                                ...
                                <tr>
                                        ...
                                        <td class="text-right"><?php echo $don['qnteSort'] ?></td>
                                       <td class="text-right"><?php echo $don['nbUniteBoiss'] ?></td>
                                       <td class="text-right"><?php echo $don['puBoiss'] ?></td>
                                       <td class="text-right"><?php echo ($don['qnteSort']*$don['puBoiss']*$don['nbUniteBoiss']) ?></td>
