<?php
    ...
    $req = $bd->prepare("SELECT A.idSortDiv,A.idDiv,date_format(A.dateSortDiv,'%e-%m-%Y à %T') as dateSortDiv,A.qteSortDiv, A.ptSortDiv, A.poste, C.idDiv,C.designation, C.pu FROM sortiediv as A, stockdivers as C WHERE  A.idDiv = C.idDiv AND A.dateSortDiv LIKE '%$dateJ%' ");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(qteSortDiv*pu) AS total FROM sortiediv as A, stockdivers as C WHERE  A.idDiv = C.idDiv AND A.dateSortDiv LIKE '%$dateJ%' ");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT A.idSortDiv,A.idDiv,date_format(A.dateSortDiv,'%e-%m-%Y à %T') as dateSortDiv,A.qteSortDiv, A.ptSortDiv, A.poste, C.idDiv,C.designation, C.pu FROM sortiediv as A,service as B, stockdivers as C WHERE A.idDiv = C.idDiv AND A.dateSortDiv BETWEEN ? AND ? ");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(qteSortDiv*pu) AS total FROM sortiediv as A,service as B, stockdivers as C WHERE A.idDiv = C.idDiv AND A.dateSortDiv BETWEEN ? AND ?");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h2 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche de sorties produits divers
                        </h2>
                    ...
                    <table class="table table-bordered table-striped spacer">
                        ...
                        <tbody>
                                ...
                                <tr>
                                       ...
                                       <td class="text-right"><?php echo $don['qteSortDiv'] ?></td>
                                       
                                       <td class="text-right"><?php echo $don['pu'] ?></td>
                                        <td class="text-right"><?php echo $don['dateSortDiv'] ?></td>

                                       
                                       <td class="text-right"><?php echo ($don['qteSortDiv']*$don['pu']) ?></td>
                                        <td></td>
