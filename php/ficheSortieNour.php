...
<?php
    ...
    $req = $bd->prepare("SELECT C.qnteSortNour,date_format(C.dateSortNour,'%e-%m-%Y à %T') as dateSortNour,A.libPv, B.designNour,B.puNour, B.idNour FROM sortienour as C,prodNour as B,pointvente as A WHERE A.idPv=C.idPv AND C.idNour=B.idNour AND C.dateSortNour LIKE '%$dateJ%' ");
        $req->execute();
        $req1 = $bd->prepare("SELECT SUM(qnteSortNour*puNour) AS total FROM sortienour as C,prodnour as B,pointvente as A WHERE A.idPv=C.idPv AND C.idNour=B.idNour AND  C.dateSortNour LIKE '%$dateJ%' ");
        $req1->execute();
    ...
    if (isset($_GET['save'])) {
        ...
        $req = $bd->prepare("SELECT C.qnteSortNour,date_format(C.dateSortNour,'%e-%m-%Y à %T') as dateSortNour,A.libPv, B.designNour,B.puNour, B.idNour FROM sortienour as C,prodNour as B,pointvente as A WHERE A.idPv=C.idPv AND C.idNour=B.idNour AND  C.dateSortNour BETWEEN ? AND ?");
        $req->execute(array($in,$out));
        $req1 = $bd->prepare("SELECT SUM(qnteSortNour*puNour) AS total FROM sortienour as C,prodnour as B,pointvente as A WHERE A.idPv=C.idPv AND C.idNour=B.idNour AND  C.dateSortNour BETWEEN ? AND ? ");
            ...
            <div class="row" style="border-bottom:1px solid black;margin-bottom:2px">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8" >
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-top:70px;">
                        <h2 class="text-center text-uppercase" style="text-align:center; border: 1px solid black">Fiche Sortie nourritures
                        </h2>
        ...
        <div class="container spacer">
            <div class="row spacer">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <table class="table table-bordered table-striped table-sm spacer">
                        <thead>
                        ...
                        <tbody>
                                ...
                                <tr>
                                        ...
                                        <td><?php echo $don['dateSortNour'] ?></td>
                                        <td class="text-right"><?php echo $don['qnteSortNour'] ?></td>
                                        <td class="text-right"><?php echo $don['puNour'] ?></td>
                                        <td class="text-right"><?php echo ($don['qnteSortNour']*$don['puNour']) ?></td>
